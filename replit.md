...
## Recent Changes
- 홈페이지 도구 카드 시각적 개선 및 링크 수정 (2025-01-29)
  - 모든 도구 카드에 컬러풀한 그라디언트 배경 적용으로 시각적 어필 향상
  - 각 도구별 고유 색상 지정 (계산기: 파란색, 포모도로: 빨간색, 날짜계산기: 초록색 등)
  - 아직 구현되지 않은 도구들에 "곧 출시 예정" 표시 추가
  - 번호 추첨기 링크를 /raffle에서 /number-raffle로 수정
  - 카드 호버 효과와 다크 모드 호환성 개선
- 서비스 소개 섹션 대폭 확장 (2025-01-29)
  - 서비스 개요 박스 추가로 ToolHub.tools 철학과 목적 상세 설명
  - 미션과 비전 카드 확대 및 내용 풍부화
  - 4가지 핵심 가치 섹션 신규 추가 (무료 서비스, 보안/프라이버시, 접근성, 사용자 중심)
  - 각 가치별 상세 설명과 고유 색상 디자인 적용
  - 콘텐츠 양 대폭 증가로 SEO 최적화 및 Google AdSense 승인 준비
